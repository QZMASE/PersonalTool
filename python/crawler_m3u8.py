...
class URL:
    ...
    def __init__(self, url):
        ...
        self.html = self.requests_url(self.url)
    ...
    def get_m3u8(self, lists):
        ...
        if self.requests_flag == True:
            ...
            m3u8_text = self.requests_url(m3u8_url)
            if self.requests_flag == True:
                m3u8_text = m3u8_text.text.split('\n')
...
if __name__=='__main__':
    ...
    for line in lines:
        if "http" in line:
            url = URL(line)
            url.get_m3u8(url.find(r'<iframe src="([^"]+\.m3u8)"'))# 爬取网页内m3u8地址
